...
class ExperimentWindow extends Component {
    ...
    if (policy !== 'no' && policy !== 'preview') {
      script.plugins = [
        ...script.plugins,
        {
          ...
          url: `/.netlify/functions/internal/?user=${user}&template=${template}&task=${task}&study=${study}&policy=${policy}&type=REVIEW&version=${version}`,
          callbacks: {},
        },
      ];
    }
  ...
  componentWillUnmount() {
    try {
      ...
      // alert(
      //   'The study has been interrupted. Please use the ESC button to stop the task!'
      // );
  ...
  render() {
    return (
      <>
        <Head>
          <link href="/static/lab-preview.css" rel="stylesheet" />
        </Head>

        <div className="container previewscreen" data-labjs-section="main">
