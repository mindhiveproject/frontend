          ...
          "content": {
            ...
            "messageHandlers": {
              "before:prepare": function anonymous() {
                ...
                let conditions = Array.from(new Set(initParameters.map(p => p.condition)));
                let conditionsParameters = {};
                for (let c of conditions) {
                	conditionsParameters[c] = initParameters.filter(p => p.condition == c)
                }
                let numberConditionsTrials = Math.ceil(numberTrials / conditions.length);
...
const responses = [... new Set(this.parameters.stimuli.map( e => {return(e.response)}))];
          ...
          "content": {
            ...
            "messageHandlers": {
              "before:prepare": function anonymous() {
                ...
                // balance design across conditions
                let conditions = Array.from(new Set(initialParameters.map(p => p.condition)));
                let conditionsParameters = {};
                for (let c of conditions) {
