        ...
        { ( { user: {data}, toggleDashboard, localState } ) => {
          ...
          return (
           <CartStyles open={localState.data.dashboardOpen}>
             <header>
               <CloseButton title="close" onClick={toggleDashboard}>&times;</CloseButton>
               <Supreme>{me.username}'s dashboard</Supreme>
               <p>You have {me.results.length} result{me.results.length === 1 ? '': 's'} </p>
             </header>
             <ul>
             {me.results.map(result => <ResultPane key={result.id} result={result} />)}
               </ul>
                 <footer>
                 <p>This is footer</p>
